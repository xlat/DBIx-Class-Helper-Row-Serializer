...
__PACKAGE__->has_many( vars => 'MyApp::Schema::Variable', 'script_id', 
					#do not serialize variables again: it's already embedded in defs
					{ is_serializable => 0 });
sub schema{ $schema }
1;
...
#write a custom unserializer to force var -> "script" relation
sub unserialize{
	my $self = shift; my ($serialized, $args, $root, $container) = @_;
	my $unserialized = $self->next::method(@_);
	#restitute "script" relation
	$unserialized->script( $container->script );
	return $unserialized;
}
